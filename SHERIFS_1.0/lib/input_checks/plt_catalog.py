...
def plt_catalog(Model_list,File_bg,catalog_file,Run_name,xmin,xmax,ymin,ymax,
...
#                    event_magnitude = np.random.triangular(cat_sig_Mw[index_cat],cat_Mw[index_cat],cat_sig_Mw_plus[index_cat])
            ...
            for mag_i,completness_i in zip(bining_in_mag,completness_used) :
                ...
                for Yr in cat_Yr: #loop on all the earthquakes in the catalogue
                    ...
                    index_cat += 1
                rate_SHARE_WCR[index_mag] = cat_model_bin[index_mag] / (end_year_of_catalog - picked_completness)  
            ...
            rate_SHARE_WCR_cum = [] 
            for i in range(len(rate_SHARE_WCR)): #calculate the cumulative rate
                rate_SHARE_WCR_cum.append(np.sum(np.array(rate_SHARE_WCR)[-(len(rate_SHARE_WCR)-i):]))
            ...
            plt.scatter(bining_in_mag,rate_SHARE_WCR_cum,c = 'brown' , marker = '_', s = 50, alpha = 0.2)#, linewidth = 0
    ...
    #            rate_minus = np.percentile(rate_SHARE_WCR_cum_density,16,axis=0)[index_mag]
                unc_rate = (np.std(rate_SHARE_WCR_cum_density,axis=0)[index_mag])/(np.square(mean_nb_eq_in_cat[index_mag]))
                ...
                rate_plus = np.array(rate_SHARE_WCR_cum_density).mean(axis=0)[index_mag]+unc_rate
                rate_minus = np.array(rate_SHARE_WCR_cum_density).mean(axis=0)[index_mag]-unc_rate
                if rate_minus < 0. : # in order not to have negative values
                    rate_minus = np.array(rate_SHARE_WCR_cum_density).mean(axis=0)[index_mag]/10.
...
#                    c='k', s=4, edgecolor='',marker = 'o',alpha = 0.8)
        plt.scatter(bining_in_mag,np.array(rate_SHARE_WCR_cum_density).mean(axis=0),
...
#                        c='k', s=5, edgecolor='',marker = 'o',alpha = 0.8)
            plt.scatter(bining_in_mag,np.array(rate_in_sub_area_cum[index_sub_area]).mean(axis=0),
                    c='k', s=15, edgecolor='',marker = 's',alpha = 0.95)
