...
class EQ_on_faults_from_sr():
    ...
    def initialize(self):
        ...
        # target stable for the GR part of the distribution
            #b = b_value
            beta = b_value * np.log(10)  
            Mf= 6.4 #Mmax with the best a priori fit, value for the Marmara sea
        ...
        number_of_loops_last_checked = -11
        empty_bins = []
        bin_target_reached = []
        ...
        while len(M_slip_budget) != 0 : # as long as there is some slip-rate to spend we keep going
            ...
            if len(empty_bins) != len(bin_mag):
                ...
                if len(fault_n_scenario_in_bin[picked_bin]) != 0:
                    ...
                    if number_of_loops > number_of_loops_last_checked + 50. or number_of_loops < 5:
                            ...
                            if len(index_fault) != 0: #it's a fault
                                ...
                                if 1. - float(sr_used)/float(sr0) >= 0.:
                                    if fault in faults_alone: #we give a boost for faults that are alone so they can  break more
                                        weight_fault.append(1.)
                                    elif fault in faults_isolated: #we give a boost for faults that are isolated so they can  break more
                                        weight_fault.append(1. - 0.5 * float(sr_used)/float(sr0))
                                    else :
                                        weight_fault.append(1. - float(sr_used)/float(sr0))
                                else:
                                ...
                                for index in index_faults_in_scenario[index_scenario[0]][0] :
                                    ...
                                    fault = faults_names[index]
                                    if fault in faults_isolated: #we give a boost for faults that are isolated so they can  break more
                                        ratio_w_i = (1. - 0.5 * float(sr_used)/float(sr0))
                                    else :
                                        ratio_w_i = (1. - float(sr_used)/float(sr0))
                                    if ratio_w_i > ratio_w:
                                        ratio_w = ratio_w_i
                                if ratio_w >= 0.:
                                    weight_fault.append(ratio_w)
                                else :
                                    weight_fault.append(0.)
